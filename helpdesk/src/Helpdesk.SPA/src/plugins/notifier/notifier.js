...
export class Notifier {
  ...
  error(title, text, timeout) {
    const model = new NotificationModel({ title, text, severity: NotificationSeverity.Error, timeout: timeout || 5000} );
    notifierEvents.$emit('show-snackbar', model);
  }

  success(title, text, timeout) {
    const model = new NotificationModel({ title, text, severity: NotificationSeverity.Success, timeout: timeout || 5000 });
    notifierEvents.$emit('show-snackbar', model);
  }

  warn(title, text, timeout) {
    const model = new NotificationModel({ title, text, severity: NotificationSeverity.Warn, timeout: timeout || 5000 });
    notifierEvents.$emit('show-snackbar', model);
  }

  info(title, text, timeout) {
    const model = new NotificationModel({ title, text, severity: NotificationSeverity.Info, timeout: timeout || 5000 });
    notifierEvents.$emit('show-snackbar', model);
  }
