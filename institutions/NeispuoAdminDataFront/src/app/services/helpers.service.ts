...
export class HelperService {
  ...
  public tableItemSelected: Subject<{ paramName: string; formDataId: string | number; additionalParams: Object }> =
    new Subject();
  ...
  addControl(
    ...
    (control as CustomFormControl).hidden = !!(
      field.hidden ||
      subsectionHidden ||
      sectionHidden ||
  ...
  transformFormValue(formValues) {
    for (let key in formValues) {
      ...
      if (formValues[key] && typeof formValues[key] === "object" && formValues[key].length > 0) {
        ...
        formValues[key] =
          formValues[key] && formValues[key].code !== undefined ? formValues[key].code : formValues[key];
      }
    }
    return formValues;
  }
  ...
  manageInfluencedField(
    ...
    if (type === InfluenceType.Options) {
    ...
    } else if (type === InfluenceType.Hide || type === InfluenceType.Render) {
      ...
      this.manageInfluencedFieldSetValue(event, influencedField, formControl, queryParams);
    }
  }
    ...
    if (!event.influence.url || !event.filterValue) {
      ...
      if (event.filterValue) {
        this.formDataService
          .getDynamicNomenclature(event.influence.url, { ...queryParams, ...filterValues })
          .subscribe(res => {
            influencedField.options = res;
            influencedField.options &&
              influencedField.options.forEach(
                option => option.isValid !== undefined && (option.disabled = !option.isValid)
              );
            this.optionsChanged.next(influencedField.name);

            const required = influencedField.validations.find(
              validation =>
                validation.name === ValidatorType.Required || validation.name === ValidatorType.RequiredControl
            );
  ...
  private manageInfluencedFieldSetValue(
    event: { value: string | number; influence: Influence; filterValue: string | number; fieldName?: string },
    influencedField: FieldConfig,
    formControl: AbstractControl,
    queryParams: Object
  ) {
    if (!event.influence.url) {
      influencedField.value = null;
      formControl.setValue(null);
    } else {
      this.formDataService
  ...
  getFormValues(group: FormGroup): Form {
    const fields = [];

    for (const key in group.value) {
      if (typeof group.value[key] !== "object" || !group.value[key]) {
        fields.push({ name: key, value: group.value[key] });
      }
    }

    return { sections: [{ subsections: [{ fields }], order: 1 }] };
  }
  ...
  private collectFilterValues(
    ...
    if (influencedBy && influencedBy.length > 1) {
      ...
      for (const fieldName of influencedBy) {
        ...
        val !== null && val !== undefined && (filter[fieldName] = val);
      }
  ...
  setFormControlsDisabled(subsection: Subsection, sectionDisabled: boolean, fg: FormGroup, parentGroup) {
    ...
    subsection.fields &&
      subsection.fields.forEach(fld => {
        ...
        formControl &&
          (sectionDisabled || subsection.disabled || fld.disabled ? formControl.disable() : formControl.enable());
      });

    subsection.subsections &&
      subsection.subsections.forEach(sub =>
        this.setFormControlsDisabled(sub, sectionDisabled || subsection.disabled, formGroup, parent)
      );
  }

  setFormControlsHidden(subsection: Subsection, sectionHidden: boolean, fg: FormGroup, parentGroup) {
    const formGroup: FormGroup = this.getFormGroup(subsection, fg);
    const parent = formGroup === fg ? parentGroup : formGroup;
    ...
    subsection.subsections &&
      subsection.subsections.forEach(sub =>
        this.setFormControlsHidden(sub, sectionHidden || subsection.hidden, formGroup, parent)
      );
  }
  ...
  hasNonHiddenField(subsection: Subsection) {
    ...
    for (const field of subsection.fields) {
      if (!field.hidden && field.rendered !== false) {
        res = true;
        break;
      }
    }
    ...
    if (!res && subsection.subsections) {
      for (const sub of subsection.subsections) {
        for (const fld of sub.fields) {
          if (!fld.hidden && fld.rendered !== false) {
            res = true;
            break;
          }
        }
