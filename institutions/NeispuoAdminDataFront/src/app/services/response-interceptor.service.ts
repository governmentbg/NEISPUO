...
export class ResponseInterceptorService implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler) {
    return next.handle(req).pipe(map(this.transformRes.bind(this, req)), catchError(this.catchErr));
  }
  ...
  private transformRes = (req, res) => {
    if (res && res.type === HttpEventType.Response) {
      if (res.body && res.body.exception) {
        ...
        if (!this.router.url.endsWith(Menu.Home)) {
  ...
  private catchErr: any = error => {
    ...
    if (hasErrorMsg) {
      ...
      this.snackBarService.openErrorSnackbar(err[0]);
    } else {
      this.snackBarService.openErrorSnackbar("Възникна грешка");
    }
