...
namespace NEISPUORegInstAPI.Controllers
{
    ...
    public class DataController : ControllerBase
    {
        ...
        //[HttpGet("file/{blobId:int}")]
        //public ActionResult downloadfile(int blobId)
        //{
        //    string HMACKey = this.configuration.GetValue<string>("AppSettings:HMACKey");
        //    string blobServiceUrl = this.configuration.GetValue<string>("AppSettings:BlobURL");
        ...
        [HttpGet("file/{institutionId:int}/{blobId:int}")]
        public ActionResult downloadfile(int? institutionId, int blobId)
        {
            ...
            Nullable<int> sysUserId = claims["SysUserID"].Type != JTokenType.Null ? (int)claims["SysUserID"] : (int?)null;
            Nullable<int> sysRoleId = claims["SysRoleID"].Type != JTokenType.Null ? (int)claims["SysRoleID"] : (int?)null;
            Nullable<int> regionId = claims["RegionID"].Type != JTokenType.Null ? (int)claims["RegionID"] : (int?)null;
            Nullable<int> municipalityId = claims["MunicipalityID"].Type != JTokenType.Null ? (int)claims["MunicipalityID"] : (int?)null;
            Nullable<int> budgetingInstitutionId = claims["BudgetingInstitutionID"].Type != JTokenType.Null ? (int)claims["BudgetingInstitutionID"] : (int?)null;

            int[] ruoRoles = { 2, 3, 9 };
            institutionId = ruoRoles.Contains((int)sysRoleId) ? institutionId : null;

            using (var connection = new SqlConnection(this.configuration.GetValue<string>("AppSettings:ConnString")))
            {
                var procname = "reginst_basic.generateUrlDocument";

                var values = new { sysUserId, sysRoleId, institutionId, regionId, municipalityId, budgetingInstitutionId, blobId, procedureId = (int?)null };
        ...
        public JsonResult GetTRData(string jsonfile, [FromBody] JsonElement data)
        {
            try
            {
                ...
                int[] directorRoles = { 0, 4, 5, 6, 7, 8, 11, 14, 20 };


                if (directorRoles.Contains((int)sysRole))
                {
        ...
        private SQLBuilderObject BuildSQL(dynamic obj, JObject data, string templateParam = "template")
        {
            ...
            foreach (var param in obj["params"])
            {
                ...
                if (!claims.ContainsKey((string)param["value"]) && Convert.ToBoolean(param["optional"]) == true && from_claim) continue;
                if (claims.ContainsKey((string)param["value"]) && claims[(string)param["value"]].ToString() == "" && Convert.ToBoolean(param["optional"]) == true && from_claim) continue;
                ...
                if (param["logical_operator"].ToString() == "and")
                {
                    ...
                    {
                        JArray arr = JArray.Parse(data[param["value"].ToString()].ToString());
