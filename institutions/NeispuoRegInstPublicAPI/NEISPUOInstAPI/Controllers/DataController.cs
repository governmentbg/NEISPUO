...
{
    //[Authorize] - QUick fix for public ui - Jani
    [Route("[controller]")]
    [ApiController]
    ...
    public class DataController : ControllerBase
    {
        ...
        [HttpGet("file/{blobId:int}/{institutionId:int}/{procedureId:int}")]
        public ActionResult downloadfile(int blobId, int institutionId, int procedureId)
        {
            using (var connection = new SqlConnection(this.configuration.GetValue<string>("AppSettings:ConnString")))
            {
                ...
                var values = new { sysUserId = (int?)null, sysRoleId = (int?)null, institutionId, regionId = (int?)null, municipalityId = (int?)null, 
                    budgetingInstitutionId = (int?)null, blobId, procedureId };
                ...
                {
                    throw new Exception(string.Format("Нямате права да извършите това действие"));
                }
            }
        }
