...
export class SubsectionComponent implements OnInit, OnChanges, OnDestroy {
  @Input() form: Form;
  ...
  @Output() filterTable: EventEmitter<string[]> = new EventEmitter();

  isLoading: boolean = false;
  checkActive: boolean = false;
  subAdded: { name: string; id: string | number };
  private subClearedSubscription: Subscription;
  ...
  ngOnInit() {
    ...
    this.subClearedSubscription = this.helperService.subCleared.subscribe((sub: { name: string; id: string | number }) => {
      if (sub.name === this.subsection.name && sub.id == this.subsection.id) {
        this.onValueChangeHelper(this.subsection);
      }
    });
  ...
  ngOnDestroy() {
    this.subClearedSubscription && this.subClearedSubscription.unsubscribe();
  }
  ...
  private changeAccordionsState() {
    ...
    if (this.subsection.subsections) {
      for (const sub of this.subsection.subsections) {
        if (
          sub.accordion &&
          ((sub.accordion.state === "closed" && !this.allClosed) || (sub.accordion.state === "opened" && this.allClosed))
        ) {
  ...
  private addSubsection(subsection: Subsection) {
    ...
    const formValues = this.helperService.customGetRawValue(this.wholeFormGroup);
  ...
  private influence(field: FieldConfig) {
    ...
    for (const influenceRecord of field.influence) {
      if (
        ...
        influenceRecord.type === InfluenceType.SetValue ||
        influenceRecord.type === InfluenceType.SetValueOppositeCondition
      ) {
        ...
        this.onValueChange({ value: field.value, influence, filterValue: field.value }, influenceRecord.type === InfluenceType.Options);
      } else {
        influence = { ...influenceRecord };
        const option = field.options ? field.options.find(option => option.code == field.value) : null;
        const label = option ? option.label : "";
        this.onValueChange({ value: field.value, influence, filterValue: null, label }, false);
      }
    }
  }
  ...
  async onValueChange(
    ...
    if (event.influence) {
        ...
        this.subsection.subsections &&
          this.subsection.subsections.forEach(subsection => {
            if (event.influence.subsectionName === subsection.name) {
              ...
              if (type === InfluenceType.Hide || type === InfluenceType.Render || type === InfluenceType.HideClear) {
                ...
                const clear = subsection.hidden && type === InfluenceType.HideClear;
                this.helperService.setFormControlsHidden(subsection, this.section.hidden, this.formGroup, this.parentGroup, clear);

                clear && this.onValueChangeHelper(subsection);
                ...
                this.helperService.setFormControlsDisabled(subsection, this.section.disabled, this.formGroup, this.parentGroup);
              }
      ...
      } else if (event.influence.sectionName || event.influence.createNewTableName) {
        const multiple = this.subsection.canDuplicate === false || this.subsection.canDuplicate === true;
  ...
  private onValueChangeHelper(subsection: Subsection) {
    if (this.subsection !== subsection) {
      this.helperService.subCleared.next({ name: subsection.name, id: subsection.id });
      return;
    }
  ...
  private loopInfluences(influencedField: FieldConfig, value, fieldName: string, initialInfluenceOptions: boolean) {
  ...
  private manageRequireInfluence(event: { value: string | number; influence: Influence; filterValue: string | number }) {
    for (let fieldName of event.influence.fields) {
      ...
      if (!influencedField) {
        this.subsection.subsections &&
          this.subsection.subsections.forEach(subsection => {
            !influencedField && (influencedField = subsection.fields.find(field => field.name === fieldName)) && (ssection = subsection);
          });
      }
        ...
        influencedField.requiredBy &&
          influencedField.requiredBy.forEach(influencer => {
            ...
            if (!inSubsection) {
              this.subsection.subsections &&
                this.subsection.subsections.forEach(subsection => {
                  !inSubsection && (inSubsection = subsection.fields.find(field => field.name === influencer.fieldName));
                });
            }
  ...
  changeSubsectionState(subsection: Subsection = null) {
    ...
    if (sub.accordion && sub.rendered !== false && !sub.hidden) {
      ...
      if (!sub.dataLoaded && sub.accordion.dataName && sub.accordion.state === "opened") {
        ...
        this.formDataService.setSubsectionAccordionData(sub, body, this.isHistory).subscribe(
          res => {
