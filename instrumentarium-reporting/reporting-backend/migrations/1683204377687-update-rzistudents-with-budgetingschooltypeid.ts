...
export class UpdateRZIStudentsWithBudgetingSchoolTypeID1682671110619
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP VIEW [reporting].[R_RZI_Students];`);
    ...
    CREATE view [reporting].[R_RZI_Students]
		...
		SELECT
			...
			ins.BudgetingSchoolTypeID AS BudgetingSchoolTypeID, --'Вид финансираща институция'
			CASE rc.InstType
				WHEN 1 THEN 'Училище'
				WHEN 2 THEN 'ДГ'
		...
		FROM
			...
			INNER JOIN core.Institution ins ON ins.InstitutionID = rc.InstId
		WHERE
			rc.InstType IN (1,2)
		GROUP BY
			rc.InstRegId,
