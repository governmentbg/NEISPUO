...
export class addLocalAreaInRStudents1695996934660
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP VIEW IF EXISTS [reporting].[R_StudentsDetails];`,
    );
    await queryRunner.query(
      `CREATE VIEW [reporting].[R_Students_Details]
    AS
       SELECT
...
`,
    ...
    await queryRunner.query(
      `DROP TABLE IF EXISTS [reporting].[R_Students_Details_Table];`,
    );
    ...
    await queryRunner.query(
      `INSERT INTO [reporting].R_Students_Details_Table WITH (TABLOCK)
        SELECT * from [reporting].R_Students_Details`,
    );
  }
