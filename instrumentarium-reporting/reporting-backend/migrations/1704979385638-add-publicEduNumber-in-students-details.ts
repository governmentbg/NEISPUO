...
export class addPublicEduNumberInRStudentsDetails1704979385638
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP VIEW IF EXISTS [reporting].[R_Students_Details];`,
    );
    await queryRunner.query(
      `CREATE VIEW [reporting].[R_Students_Details] as
SELECT 
		   ...
		   ,la.LocalAreaID
		   ,ISNULL(la.Name,'') AS LAreaName --'Район'
           ,sc.InstitutionID AS InstitutionID --'Код по НЕИСПУО'
           ,isy.BudgetingSchoolTypeID AS BudgetingSchoolTypeID -- Финансираща институция
           ...
           ,p.PublicEduNumber AS PublicEduNumber --'ЛОН'
		   ...
		   ,CASE p.Gender
				 ...
				 ELSE ''
			END AS Gender --'Пол'
		   ,tP.Name AS PersonTown--'Населено място'
		   ,mP.Name AS PersonMunicipality--'Община'
		   ,rP.Name AS PersonRegion--'Област'
		   ,c.Name AS Nationality --'Националност'
        ...
        FROM
            ...
            INNER JOIN location.Town t ON isy.TownID = t.TownID 
			...
			LEFT OUTER JOIN location.Town tP ON tP.TownID = p.CurrentTownID
			LEFT OUTER JOIN location.Municipality mP ON mP.MunicipalityID = tP.MunicipalityID
			LEFT OUTER JOIN location.Region rP ON rP.RegionID = mP.RegionID
			LEFT OUTER JOIN location.Country c ON c.CountryID = p.NationalityID
			INNER JOIN inst_basic.InstitutionDepartment id ON id.InstitutionID = isy.InstitutionId
															  AND id.IsMain = 1
			LEFT OUTER JOIN location.LocalArea la ON la.LocalAreaID = id.LocalAreaID
        WHERE
            cy.IsValid = 1
            AND sc.IsCurrent = 1
    ...
    await queryRunner.query(
      `ALTER TABLE reporting.R_Students_Details_Table
ADD PublicEduNumber nvarchar(255);`,
    );
  }
