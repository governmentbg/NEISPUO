...
export class AddRCurriculumSectionBProfessionalView1757074412000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        ...
        VALUES
        ('${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}', ${SysRoleEnum.MON_ADMIN}),
        ('${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}', ${SysRoleEnum.MON_EXPERT}),
        ('${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}', ${SysRoleEnum.RUO_EXPERT}),
        ('${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}', ${SysRoleEnum.RUO}),
        ('${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}', ${SysRoleEnum.INSTITUTION});`);

    await queryRunner.query(`
    CREATE VIEW [reporting].[R_Curriculum_Section_B_Professional]
		AS
    SELECT distinct i.InstitutionID as 'Код по НЕИСПУО'
     ...
     WHERE i.BaseSchoolTypeID in (1,2,3,7,11,12,13,14,18) 
    		...
    		and cg.ClassTypeID in (20)   ----професионална паралелка
    		and c.SubjectTypeID not in (108); --- заради коментар Без РПП
    `);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
       ...
       WHERE SchemaName='${SysSchemaEnum.R_CURRICULUM_SECTION_B_PROFESSIONAL}'`);

    await queryRunner.query(`
       DROP VIEW [reporting].[R_Curriculum_Section_B_Professional]`);
  }
}
