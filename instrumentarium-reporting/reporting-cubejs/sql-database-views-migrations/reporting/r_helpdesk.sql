...
FROM
	helpdesk.Issue i
LEFT JOIN helpdesk.Status s ON
	i.StatusId = s.Id
LEFT JOIN helpdesk.Priority p ON
	i.PriorityId = p.Id
LEFT JOIN helpdesk.Category c ON
	i.CategoryId = c.Id
LEFT JOIN helpdesk.Category sc ON
	i.SubcategoryId = sc.Id
LEFT JOIN core.SysUser su ON
	i.SubmitterSysUserId = su.SysUserID
LEFT JOIN core.SysRole sr ON
	i.SubmitterSysRoleId  = sr.SysRoleID
LEFT JOIN core.SysUser au ON
	i.AssignedToSysUserId = au.SysUserID
LEFT JOIN core.SysUser cu ON
	i.CreatedBySysUserID = cu.SysUserID
LEFT JOIN core.SysUser mu ON
	i.CreatedBySysUserID = mu.SysUserID
LEFT JOIN core.InstitutionSchoolYear isy ON
	i.InstitutionId = isy.InstitutionId
	AND i.SchoolYear = isy.SchoolYear