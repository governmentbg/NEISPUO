...
export const syncRegularAverageAbsencePerStudentJob: CronJob = {
  name: 'sync-regular-average-absence-per-student',
  schedule: SCHEDULE_CONFIG.syncRegularAverageAbsencePerStudent,
  enabled: true,
  ...
  task: async () => {
    ...
    await executeSyncJob(
      ...
      (err) => logSyncError(syncRegularAverageAbsencePerStudentJob.name, err),
    );
  },
};
