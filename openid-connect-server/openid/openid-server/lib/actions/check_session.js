...
module.exports = {
  get: async function checkSessionIframe(ctx, next) {
    const { keepHeaders, scriptNonce } = instance(ctx.oidc.provider).configuration(
...
<script ${
      nonce ? `nonce="${nonce}" ` : ''
    }src="https://cdnjs.cloudflare.com/ajax/libs/jsSHA/2.3.1/sha256.js" integrity="sha256-NyuvLfsvfCfE+ceV6/W19H+qVp3M8c9FzAgj72CW39w=" crossorigin="anonymous"></script>
<script ${
      nonce ? `nonce="${nonce}" ` : ''
    }src="https://cdnjs.cloudflare.com/polyfill/v3/polyfill.min.js?flags=gated&features=fetch"></script>
</head>
<body>
