...
const CORS = {
  ...
  impersonate: cors({
    allowMethods: 'POST',
    origin: [process.env.IMPERSONATE_ORIGIN_WHITELIST],
    // allows for session to be sent from client
    credentials: true,
    ...CORS_AUTHORIZATION,
  }),
  end_impersonate: cors({
    allowMethods: 'POST',
    origin: [process.env.IMPERSONATE_ORIGIN_WHITELIST],
    // allows for session to be sent from client
    credentials: true,
...
module.exports = function initializeApp() {
  ...
  // apply custom CORS policy for /impersonate route
  // names of requests should be different for options and post req
  options('cors.impersonate', '/impersonate', CORS.impersonate);
  post('impersonate', '/impersonate', CORS.impersonate);
  options('cors.end_impersonate', '/end_impersonate', CORS.end_impersonate);
  post('end_impersonate', '/end_impersonate', CORS.end_impersonate);
  ...
  if (configuration.features.sessionManagement.enabled) {
    get(
      ...
      error(this, 'check_session.error'),
      checkSession.get,
    );
    post(
      'check_session_origin',
      routes.check_session,
      error(this, 'check_session_origin.error'),
      ...checkSession.post,
    );
  }
  ...
  if (configuration.features.deviceFlow.enabled) {
    ...
    get(
      'device_resume',
      `${routes.code_verification}/:user_code/:uid`,
  ...
  async function proxyWarning(ctx, next) {
    ...
    if (issuer.startsWith('https:') && !ctx.secure && ctx.get('x-forwarded-proto') === 'https') {
      attention.warn(
        `x-forwarded-proto header detected but not trusted, you must set proxy=true on the provider, see documentation for more details: ${docs(
      ...
      attention.warn(
        `x-forwarded-proto header not detected for an https issuer, you must configure your ssl offloading proxy and the provider, see documentation for more details: ${docs(
          'trusting-tls-offloading-proxies',
        )}`,
      );
      proxyWarning.pass = true;
