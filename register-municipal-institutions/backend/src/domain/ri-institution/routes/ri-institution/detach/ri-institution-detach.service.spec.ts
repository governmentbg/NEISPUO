...
describe('Ri-institutionDetachService', () => {
    let service: RIInstitutionDetachService;
    ...
    const mockRIInstitutionService = {
        generateInstitutionID: jest.fn().mockResolvedValue({ Town: 'TownToCreate', InstitutionID: undefined }),
    ...
    beforeEach(async () => {
        ...
        service = module.get<RIInstitutionDetachService>(RIInstitutionDetachService);
    });

    it('should successfully detach an institutions', async () => {
        const body = {
            misToCreate: [{
                InstitutionID: 0,
                RIProcedure: {
...
miToUpdate: {
        ...
        const result = await service.detachInstitutions(authedRequest as any, body);
        expect(mockTransformTypeRepository.findOne).toHaveBeenCalledWith(13);
        expect(mockTransformTypeRepository.findOne).toHaveBeenCalledTimes(3);
        expect(mockProcedureTypeRepository.findOne).toHaveBeenCalledWith(2);
        expect(mockProcedureTypeRepository.findOne).toHaveBeenCalledTimes(3);
        expect(mockStatusTypeRepository.findOne).toHaveBeenCalledWith(4);
        expect(mockStatusTypeRepository.findOne).toHaveBeenCalledTimes(3);
        ...
        const riInstitution = await mockRIInstitutionRepository.save(body.miToUpdate);
        expect(riInstitution).toEqual({
            Name: 'RiInstitution-name',
            Abbreviation: 'Abbreviation',
        });
...
value: 'flex1',
        ...
        expect(mockRIInstitutionService.generateInstitutionID).toHaveBeenCalledWith(body.misToCreate[0].Town, undefined);
        expect(mockRIInstitutionService.azureSyncCreateSchool).toHaveBeenCalledWith({
            Abbreviation: 'Abbreviation',
            Name: 'RiInstitution-name',
        }, 'Authorization Bearer');

        expect(result).toEqual([{ Abbreviation: 'Abbreviation', Name: 'RiInstitution-name' },
        { Abbreviation: 'Abbreviation', Name: 'RiInstitution-name' }]);
    });
});
