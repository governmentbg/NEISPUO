...
describe('Ri-institutionDivideService', () => {
    let service: RIInstitutionDivideService;
    ...
    beforeEach(async () => {
        ...
        service = module.get<RIInstitutionDivideService>(RIInstitutionDivideService);
    });

    it('should successfully divide an institutions', async () => {
        const body = {
            misToCreate: [{
                RIProcedure: {
                    __name__: 'RIProcedureFromBody',
...
miToDelete: {
        ...
        const result = await service.divideInstitutions(authedRequest as any, body);
        expect(mockTransformTypeRepository.findOne).toHaveBeenCalledWith(14);
        ...
        expect(mockRIProceduresRepository.find).toHaveBeenCalledWith({ InstitutionID: body.miToDelete.InstitutionID });
        ...
        const riInstitution = await mockRIInstitutionRepository.save(body.misToCreate);
