...
@Component({
  selector: 'app-detach-mi-modal-preview',
  templateUrl: './detach-mi-modal-preview.component.html',
  styleUrls: ['./detach-mi-modal-preview.component.scss'],
})
export class DetachMIModalPreviewComponent implements OnInit, AfterViewInit, IStatePage {
  confirmDialog: any
  ...
  subs = new SubSink();
  isBulstatLookupEnabled: boolean = !this.dpService.isEditMode;
    ...
    .pipe(
      ...
      map(
        (baseSchoolTypes) => baseSchoolTypes
          .filter((bst) => bst.BaseSchoolTypeID === 15 || bst.BaseSchoolTypeID === 16),
