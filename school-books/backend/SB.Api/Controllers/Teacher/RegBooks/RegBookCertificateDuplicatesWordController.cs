...
public class RegBookCertificateDuplicatesWordController : RegBooksWordController
{
    ...
    public async Task<FileStreamResult> DownloadWordFileAsync(
        ...
        [FromServices] IRegBookCertificateDuplicateQueryRepository regBookCertificateDuplicateQueryRepository,
        ...
        CancellationToken ct)
    {
        ...
        var record = await regBookCertificateDuplicateQueryRepository.GetWordDataAsync(schoolYear, recordId, ct);
        var jsonObject = JsonSerializer.Serialize(
            new
            {
                ...
                BookName = "издадените дубликати на удостоверения"
            },
        ...
        return new FileStreamResult(wordStream, OpenXmlExtensions.WordMimeType) { FileDownloadName = $"Рег_кн_изд_серт_дубл_{record.RegistrationNumberTotal}-{record.RegistrationNumberYear}/{record.RegistrationDate?.ToString("dd.MM.yyyy") ?? string.Empty}_подпис.docx" };
    }
}
