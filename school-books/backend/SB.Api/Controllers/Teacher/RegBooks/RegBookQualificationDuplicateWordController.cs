...
public class RegBookQualificationDuplicatesWordController : RegBooksWordController
{
    ...
    public async Task<FileStreamResult> DownloadWordFileAsync(
        ...
        [FromServices] IRegBookQualificationDuplicateQueryRepository regBookQualificationDuplicateQueryRepository,
        ...
        CancellationToken ct)
    {
        ...
        var record = await regBookQualificationDuplicateQueryRepository.GetWordDataAsync(schoolYear, recordId, ct);
        var jsonObject = JsonSerializer.Serialize(
            new
            {
                ...
                BookName = "издадените дубликати на документи за завършена степен на образование и за придобита професионална квалификация"
            },
        ...
        return new FileStreamResult(wordStream, OpenXmlExtensions.WordMimeType) { FileDownloadName = $"Рег_кн_изд_док_дубл_{record.RegistrationNumberTotal}-{record.RegistrationNumberYear}/{record.RegistrationDate?.ToString("dd.MM.yyyy") ?? string.Empty}_подпис.docx" };
    }
}
