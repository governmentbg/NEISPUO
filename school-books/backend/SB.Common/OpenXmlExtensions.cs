...
public static partial class OpenXmlExtensions
{
    ...
    public static int AppendBorder(
        ...
        bool bottom = true)
    {
        Border cellBorders = new();
        if (left)
        {
            cellBorders.LeftBorder = new(new Color() { Auto = true }) { Style = borderStyle };
        }
        if (right)
        {
            cellBorders.RightBorder = new(new Color() { Auto = true }) { Style = borderStyle };
        }
        if (top)
        {
            cellBorders.TopBorder = new(new Color() { Auto = true }) { Style = borderStyle };
        }
        if (bottom)
        {
            cellBorders.BottomBorder = new(new Color() { Auto = true }) { Style = borderStyle };
        }

        var borders = stylesheet.GetFirstChild<Borders>()
            ?? throw new Exception("Missing element Borders. Has the sheet been initialized?");
        ...
        int? borderId = null)
    {
            ...
            borderId != null)
        {
            ...
            {
                cellFormat.BorderId = (uint)borderId.Value;
            }
        }
