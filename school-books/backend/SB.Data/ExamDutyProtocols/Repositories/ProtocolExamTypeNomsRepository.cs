...
internal class ProtocolExamTypeNomsRepository : Repository, IScopedEntityNomsRepository<ProtocolExamType>
{
    public ProtocolExamTypeNomsRepository(UnitOfWork unitOfWork)
        : base(unitOfWork)
    {
    }
    ...
    public async Task<NomVO[]> GetNomsByIdAsync(int schoolYear, int instId, int[] ids, CancellationToken ct)
    {
        ...
        return await this.DbContext.Set<ProtocolExamType>()
            .Where(s =>
                this.DbContext
                    .MakeIdsQuery(ids)
                    .Any(id => s.Id == id.Id))
    ...
    {
        var predicate = PredicateBuilder.True<ProtocolExamType>();
