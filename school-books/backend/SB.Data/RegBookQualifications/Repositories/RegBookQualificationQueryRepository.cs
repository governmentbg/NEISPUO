...
internal class RegBookQualificationQueryRepository : Repository, IRegBookQualificationQueryRepository
{
    public RegBookQualificationQueryRepository(UnitOfWork unitOfWork)
        : base(unitOfWork)
    {
    }
    ...
    {
        var predicate = PredicateBuilder.True<RegBookQualification>();
        ...
        return await (
            from rbq in this.DbContext.Set<RegBookQualification>().Where(predicate)
        ...
        CancellationToken ct)
    {
        var duplicates = await
            (from d in this.DbContext.Set<RegBookQualificationDuplicate>()
        ...
        return await
            (from rbq in this.DbContext.Set<RegBookQualification>()
    ...
    public async Task<GetExcelDataVO[]> GetExcelDataAsync(int schoolYear, int instId, int basicDocumentId, CancellationToken ct)
    {
        var origDocumentData = await
            (from rbq in this.DbContext.Set<RegBookQualification>()
             ...
             select new
             {
                 ...
                 bd.Name,
                 EduFormName = ef.Name,
                 ClassTypeName = ctype.Name,
                 SPPOOProfessionName = prof.Name,
                 SPPOOSpecialityName = spec.Name,
                 rbq.Gpa,
                 rbq.Series,
                 rbq.FactoryNumber,
                 rbq.IsCancelled,
        ...
        (
            ...
            od.EduFormName,
            od.ClassTypeName,
            od.SPPOOProfessionName,
            od.SPPOOSpecialityName,
            od.Gpa,
            od.Series,
            od.FactoryNumber,
            od.IsCancelled,
            duplicates[od.Id].Select(d => new GetExcelDataVODuplicate(
                d.RegistrationDate,
                d.Series + '-' + d.FactoryNumber
            )).ToArray()
        )).ToArray();
    }
