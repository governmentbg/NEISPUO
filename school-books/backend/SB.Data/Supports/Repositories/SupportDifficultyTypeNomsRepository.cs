...
internal class SupportDifficultyTypeNomsRepository : Repository, IScopedEntityNomsRepository<SupportDifficultyType>
{
    public SupportDifficultyTypeNomsRepository(UnitOfWork unitOfWork)
        : base(unitOfWork)
    {
    }
    ...
    public async Task<NomVO[]> GetNomsByIdAsync(int schoolYear, int instId, int[] ids, CancellationToken ct)
    {
        ...
        return await this.DbContext.Set<SupportDifficultyType>()
            .Where(s =>
                this.DbContext
                    .MakeIdsQuery(ids)
                    .Any(id => s.Id == id.Id))
    ...
    {
        var predicate = PredicateBuilder.True<SupportDifficultyType>();
