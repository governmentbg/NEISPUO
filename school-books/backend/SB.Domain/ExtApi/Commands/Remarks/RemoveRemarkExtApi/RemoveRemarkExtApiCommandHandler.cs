...
internal record RemoveRemarkExtApiCommandHandler(
    IUnitOfWork UnitOfWork,
    IScopedAggregateRepository<Remark> RemarkAggregateRepository,
    IQueueMessagesService QueueMessagesService,
    IClassBookCachedQueryStore ClassBookCachedQueryStore)
    : IRequestHandler<RemoveRemarkExtApiCommand>
{
    public async Task Handle(RemoveRemarkExtApiCommand command, CancellationToken ct)
    {
        ...
        this.RemarkAggregateRepository.Remove(remark);

        await this.QueueMessagesService.CancelMessagesAndSaveAsync<NotificationQueueMessage>(remark.EmailTag, ct);
        await this.QueueMessagesService.CancelMessagesAndSaveAsync<NotificationQueueMessage>(remark.PushNotificationTag, ct);
