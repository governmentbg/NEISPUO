...
internal record CreateRemarkCommandHandler(
    ...
    : IRequestHandler<CreateRemarkCommand, int>
{
    public async Task<int> Handle(CreateRemarkCommand command, CancellationToken ct)
    {
        ...
        var curriculumName = await this.NotificationsQueryRepository.GetCurriculumNameAsync(schoolYear, remark.CurriculumId, ct);
        var jObject = JObject.FromObject(new
        {
            remarkTypeText = remark.Type.GetEnumDescription()!.ToLower(),
            note = remark.Description,
            curriculumName,
        });

        await this.NotificationsService.TryPostNotificationsAsync("NewRemark", remark.PersonId, jObject, ct);
