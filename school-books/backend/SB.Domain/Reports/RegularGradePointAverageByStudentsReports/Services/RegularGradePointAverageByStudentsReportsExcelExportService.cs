...
public class RegularGradePointAverageByStudentsReportsExcelExportService : IRegularGradePointAverageByStudentsReportsExcelExportService
{
    private readonly IRegularGradePointAverageByStudentsReportsQueryRepository regularGradePointAverageByStudentsReportsQueryRepository;

    public RegularGradePointAverageByStudentsReportsExcelExportService(
        IRegularGradePointAverageByStudentsReportsQueryRepository regularGradePointAverageByStudentsReportsQueryRepository)
    {
        this.regularGradePointAverageByStudentsReportsQueryRepository = regularGradePointAverageByStudentsReportsQueryRepository;
    }
        ...
        CancellationToken ct)
    {
        ...
        var worksheetPart = workbookPart.AddNewPart<WorksheetPart>("regularGradePointAverageByStudentsReport");
        ...
        sheets.AppendChild(
            new Sheet()
            {
                ...
                Name = "среден успех от текущи оценки"
            });
    }
    ...
    {
        var report =
            await this.regularGradePointAverageByStudentsReportsQueryRepository.GetExcelDataAsync(
                schoolYear,
                instId,
                regularGradePointAverageByStudentsReportId,
                ct);
        ...
        {
            "Клас", "Ученик", "Предмет", "Успех", "Оценки", "Слаб 2", "Среден 3", "Добър 4", "Мн. Добър 5", "Отличен 6"
        };
        ...
        {
            titleRowText = $"Среден успех от текущи оценки по ученици от {report.ClassBookNames} за {report.Period.ToLower()} към дата {report.CreateDate: dd.MM.yyyy HH:mm}";
        }
        else
        {
            titleRowText = $"Среден успех от текущи оценки по ученици за {report.Period.ToLower()} към дата {report.CreateDate: dd.MM.yyyy HH:mm}";
        }
