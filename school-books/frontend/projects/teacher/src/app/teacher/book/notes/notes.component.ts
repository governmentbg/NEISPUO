...
import { NotesService, Notes_GetAll } from 'projects/sb-api-client/src/api/notes.service';
...
export class NotesComponent {
  ...
  dataSource: TableDataSource<Notes_GetAll>;
  canCreate = false;
  ...
  constructor(
    ...
    this.dataSource = new TableDataSource((sortBy, sortDirection, offset, limit) =>
      notesService.getAll({ schoolYear, instId, classBookId, offset, limit })
    );

    // the promise should be resolved by this stage,
    // so no need for a skeleton screen
    classBookInfo.then((classBookInfo) => {
      this.canCreate = classBookInfo.bookAllowsModifications && classBookInfo.hasCreateNoteAccess;
    });
  }
}
