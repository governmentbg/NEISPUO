...
@Component({
  selector: 'sb-replr-participations',
  templateUrl: './replr-participations.component.html'
})
export class ReplrParticipationsComponent {
  readonly fasPlus = fasPlus;
  readonly fadChevronCircleRight = fadChevronCircleRight;

  dataSource: TableDataSource<ReplrParticipations_GetAll>;
  canCreate = false;
  ...
  constructor(
    ...
    this.dataSource = new TableDataSource((sortBy, sortDirection, offset, limit) =>
      replrParticipationsService.getAll({ schoolYear, instId, classBookId, offset, limit })
    );

    // the promise should be resolved by this stage,
    // so no need for a skeleton screen
    classBookInfo.then((classBookInfo) => {
      this.canCreate = classBookInfo.bookAllowsModifications && classBookInfo.hasCreateReplrParticipationAccess;
    });
  }
}
