...
import { SanctionsService, Sanctions_GetAll } from 'projects/sb-api-client/src/api/sanctions.service';
...
export class SanctionsComponent {
  ...
  dataSource: TableDataSource<Sanctions_GetAll>;
  canCreate = false;
  classBookId: number;
  ...
  constructor(
    ...
    this.dataSource = new TableDataSource((sortBy, sortDirection, offset, limit) =>
      sanctionsService.getAll({ schoolYear, instId, classBookId: this.classBookId, offset, limit })
    );

    // the promise should be resolved by this stage,
    // so no need for a skeleton screen
    classBookInfo.then((classBookInfo) => {
      this.canCreate = classBookInfo.bookAllowsModifications && classBookInfo.hasCreateSanctionAccess;
    });
  }
}
