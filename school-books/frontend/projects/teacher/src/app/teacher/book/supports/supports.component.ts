...
import { SupportsService, Supports_GetAll } from 'projects/sb-api-client/src/api/supports.service';
...
export class SupportsComponent {
  ...
  dataSource: TableDataSource<Supports_GetAll>;
  canCreate = false;
  ...
  constructor(
    ...
    this.dataSource = new TableDataSource((sortBy, sortDirection, offset, limit) =>
      supportsService.getAll({ schoolYear, instId, classBookId, offset, limit })
    );

    // the promise should be resolved by this stage,
    // so no need for a skeleton screen
    classBookInfo.then((classBookInfo) => {
      this.canCreate = classBookInfo.bookAllowsModifications && classBookInfo.hasCreateSupportAccess;
    });
  }
}
