...
import { getFirstTabRoute } from 'projects/shared/utils/book';
import { throwParamError, tryParseInt } from 'projects/shared/utils/various';
import { map } from 'rxjs/operators';
import { AbsencesDplrSkeletonComponent, AbsencesDplrType } from './absences-dplr/absences-dplr.component';
...
import { StudentBookSkeletonComponent } from './student-book/student-book.component';
...
@Injectable({ providedIn: 'root' })
export class RetirectToDefaultTab implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    return this.classBooksService
      ...
      .pipe(map((classBookInfo) => createUrlTreeFromSnapshot(route, [getFirstTabRoute(classBookInfo)])));
  }
}

const routes: Routes = [
  {
    path: ':studentClassBookId',
    component: StudentBookSkeletonComponent,
    children: [
      { path: '', pathMatch: 'full', canActivate: [RetirectToDefaultTab], children: [] },
      {
        path: 'grades',
      ...
      },
      {
        ...
        data: {
          type: AbsencesDplrType.Absence
        }
      },
      {
        path: 'attendances-dplr',
        component: AbsencesDplrSkeletonComponent,
        data: {
          type: AbsencesDplrType.Attendance
        }
      },
      {
        path: 'topics',
