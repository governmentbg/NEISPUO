...
const storageKey = 'auth-neispuo-students';
...
class SecureStorageStateStore {
    ...
    set(key, value) {
        ...
        const store = CryptoJS.AES.encrypt(value, encryptionToken).toString();
        console.log(store);
    ...
    get(key) {
        ...
        if (item) {
          try {
            ...
            const bytes = CryptoJS.AES.decrypt(item, encryptionToken);
