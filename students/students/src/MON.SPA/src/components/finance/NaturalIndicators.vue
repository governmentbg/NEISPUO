      ...
      <v-card-title>
        <v-col
          ...
          >
            <autocomplete
              v-model="periods"
              api="/api/lookups/GetNaturalIndicatorsPeriods"
              :label="$t('finance.naturalIndicators.periods')"
          ...
          >
            <v-checkbox
              ...
              :label="$t('finance.naturalIndicators.showItemPrice')"
            />
        </v-col>         
          <v-col
            cols="12"
          ...
          >
            <v-checkbox
              v-model="showItemValue"
              :label="$t('finance.naturalIndicators.showItemValue')"
            />
        </v-col>             
        <v-spacer />
        <v-text-field
...
export default {
  name: "NaturalIndicatorsListComponent",
  components: {
    Autocomplete,
    GridExporter
  },
  ...
  computed: {
    ...
    hasManagePermission() {
      return this.hasPermission(Permissions.PermissionNameForNaturalIndicatorsManage);
    },
    periodsSelected() {
  ...
  async beforeMount() {
    ...
    this.$api.finance.getGridHeaders().then((response) => {
