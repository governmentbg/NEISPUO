...
namespace MON.Report.Service.Diploma
{
    public class Diploma_3_44ReportService : DiplomaReportBaseService<Diploma_3_44ReportService>
    {
        ...
        public override object LoadReport(IDictionary<string, object> parameters)
        {
            ...
            model.MandatoryBasicDocumentPartId = 152;
            ...
            {
                var institution = _db.Institutions.FirstOrDefault(i => i.InstitutionId == firstHighSchoolLevel.InstitutionId);
            ...
            // Изключваме БЕЛ
            var subjectTypeIdsForExternalEvaluation = diploma.Grades.Where(i => i.ExternalEvaluationTypeId == (int)ExternalEvaluationTypeEnum.DZI && i.SubjectId != (int)SubjectsEnum.BEL).Select(i => i.SubjectTypeId).Distinct();
