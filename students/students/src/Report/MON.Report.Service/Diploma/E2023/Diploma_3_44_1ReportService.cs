namespace MON.Report.Service.Diploma.E2023
{
    ...
    public class Diploma_3_44_1ReportService : DiplomaReportBaseService<Diploma_3_44_1ReportService>
    {
        public Diploma_3_44_1ReportService(DbReportServiceDependencies<Diploma_3_44_1ReportService> dependencies) :
            base(dependencies)
        {
        }
        ...
        public override object LoadReport(IDictionary<string, object> parameters)
        {
            ...
            diploma.SpecialityForeign = _db.Sppoospecialities.FirstOrDefault(i => i.SppoospecialityCode == diploma.SpecialityCode && i.IsValid == true)?.NameEn ?? null;
            diploma.ProfessionForeign = _db.Sppooprofessions.FirstOrDefault(i => i.SppooprofessionCode == diploma.ProfessionCode && i.IsValid == true)?.NameEn ?? null;
            diploma.Ministry = _db.Ministries.FirstOrDefault(i => i.Id == dbDiploma.MinistryId)?.NameEn;
            ...
            diploma.EduFormForeign = _db.EduForms.FirstOrDefault(i => i.ClassEduFormId == dbDiploma.EduFormId)?.NameEn;
            diploma.ClassType = _db.ClassTypes.FirstOrDefault(i => i.ClassTypeId == diploma.ClassTypeId)?.NameEn ?? null;
            ...
            foreach (DiplomaGradeModel grade in diploma.Grades)
            {
                ...
                if (!String.IsNullOrWhiteSpace(subject?.NameEn))
                {
                    grade.SubjectName = subject.NameEn;
                }
            ...
            {
                diploma.SubjectTypesDetails.Add(new SubjectTypeDetail() { Name = "GES", Description = "taught as general education subject" });
            }
            if (subjectTypeIdsForExternalEvaluation.Intersect(GlobalConstants.SubjectTypesOfProfileSubject).Any())
            {
                diploma.SubjectTypesDetails.Add(new SubjectTypeDetail() { Name = "PS", Description = "studied as a profiling subject" });
            }
        ...
        private string GetDecimalGradeText(decimal? gradeValue)
        {
            ...
            {
                var gv when gv < 3.5m => "Satisfactory",
                var gv when gv < 4.5m => "Good",
                var gv when gv < 5.5m => "Very Good",
