...
namespace MON.Report.Service.Diploma.E2024
{
    public class Diploma_3_30ReportService : DiplomaReportBaseService<Diploma_3_30ReportService>
    {
        ...
        public override object LoadReport(IDictionary<string, object> parameters)
        {
            ...
            List<int> PrintedSubjectIds = new List<int>() { 1, 2, 91, 18, 13, 27, 28, 25, 29, 30, 31, 72, 45 };
            ...
            foreach (var grade in diploma.Grades)
            {
                grade.GradeText = grade.GradeText?.Replace("Постига изискванията", "ПИ");
                grade.GradeText = grade.GradeText?.Replace("Справя се", "СС");
                grade.GradeText = grade.GradeText?.Replace("Среща затруднения", "СЗ");
            }
            ...
            // Взимаме секцията ЗУЧ за 7-ми клас, за да разберем кой е езика, който да разпечатаме
            int? basicDocumentPartCategory = GetBasicDocumentPartCategory(diploma.BasicDocumentId, BasicDocumentPartCategoryEnum.MandatoryPart, 7);
            List<DiplomaGradeModel> flGrade = diploma.Grades
                .Where(i => i.DocumentPartId == basicDocumentPartCategory
                && i.SubjectId != null
