namespace MON.Report.Service.Diploma.E2025
{
    ...
    public class Diploma_3_31ReportService : DiplomaReportBaseService<Diploma_3_31ReportService>
    {
        public Diploma_3_31ReportService(DbReportServiceDependencies<Diploma_3_31ReportService> dependencies)
            : base(dependencies)
        {
        ...
        public override object LoadReport(IDictionary<string, object> parameters)
        {
            ...
            // Зареждане на оценките от НВО за 10-ти клас
            ...
            // За 3-31 ни трябват само 2 чужди предмета
            diploma.FLGrades = diploma.FLGrades.Take(2).ToList();
            ...
            diploma.MandatoryAdditionalGrades = diploma.MandatoryGrades.Where(i => i.ExternalEvaluationTypeId == null 
                && (i.SubjectId == null 
                || (!PrintedSubjectIds.Contains(i.SubjectId.Value) && !diploma.FLGrades.Any(x=> x.SubjectId == i.SubjectId.Value))))
                .ToList();
