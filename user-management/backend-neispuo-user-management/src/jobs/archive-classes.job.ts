...
@Injectable()
export class ArchiveClassesJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveClassesJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_AZURE_CLASSES, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_AZURE_CLASSES,
    })
