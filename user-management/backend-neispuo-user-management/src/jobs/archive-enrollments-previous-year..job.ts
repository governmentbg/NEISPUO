...
@Injectable()
export class ArchiveEnrollmentsPreviousYearJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveEnrollmentsPreviousYearJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_ENROLLMENTS_PREVIOUS_YEARS, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_ENROLLMENTS_PREVIOUS_YEARS,
    })
        ...
        if (this.previousJobHasFinished) {
            ...
            try {
                ...
                await this.azureArchiveService.archiveEntities({
                    entity: ArchivableEntity.ENROLLMENTS,
