...
@Injectable()
export class ArchiveEnrollmentsJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveEnrollmentsJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_AZURE_ENROLLMENTS, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_AZURE_ENROLLMENTS,
    })
