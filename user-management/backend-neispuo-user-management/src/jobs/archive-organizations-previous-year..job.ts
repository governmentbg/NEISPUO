...
@Injectable()
export class ArchiveOrganizationsPreviousYearJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveOrganizationsPreviousYearJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_ORGANIZATIONS_PREVIOUS_YEARS, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_ORGANIZATIONS_PREVIOUS_YEARS,
    })
        ...
        if (this.previousJobHasFinished) {
            ...
            try {
                ...
                await this.azureArchiveService.archiveEntities({
                    entity: ArchivableEntity.ORGANIZATIONS,
