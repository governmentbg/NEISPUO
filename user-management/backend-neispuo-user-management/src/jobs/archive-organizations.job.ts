...
@Injectable()
export class ArchiveOrganizationsJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveOrganizationsJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_AZURE_ORGANIZATIONS, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_AZURE_ORGANIZATIONS,
    })
