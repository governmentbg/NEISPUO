...
@Injectable()
export class ArchiveUsersJob {
    constructor(private azureArchiveService: AzureArchiveService) {}

    private readonly logger = new Logger(ArchiveUsersJob.name);

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_ARCHIVE_AZURE_USERS, {
        name: CONSTANTS.JOB_NAME_ARCHIVE_AZURE_USERS,
    })
