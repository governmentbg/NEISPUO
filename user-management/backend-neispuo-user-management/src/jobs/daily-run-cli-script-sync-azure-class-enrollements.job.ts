...
@Injectable()
export class DailyRunCliScriptSyncAzureClassEnrollmentsJob {
    constructor(private syncEnrollmentsService: SyncEnrollmentsService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_DAILY_RUN_CLI_SCRIPT_SYNC_AZURE_CLASS_ENROLLMENTS, {
        name: CONSTANTS.JOB_NAME_DAILY_RUN_CLI_SCRIPT_SYNC_AZURE_CLASS_ENROLLMENTS,
    })
    ...
    async handleCron() {
        if (this.previousJobHasFinished) {
            ...
            try {
                console.log(`syncClassEnrollments`);
                await this.syncEnrollmentsService.syncNotRecievedAzureClassEnrollments();
            } catch (e) {
