...
@Injectable()
export class SendAzureClassesCreateJob {
    constructor(private azureClassesService: AzureClassesService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_CLASSES_CREATE, { name: CONSTANTS.JOB_NAME_SEND_AZURE_CLASSES_CREATE })
    @RunOnDeployment({ names: [DeploymentGroup.SEND] })
    async handleCron() {
        if (this.previousJobHasFinished) {
            this.previousJobHasFinished = false;
            try {
                await this.azureClassesService.sendAzureClasses(WorkflowType.CLASS_CREATE);
            } catch (e) {
