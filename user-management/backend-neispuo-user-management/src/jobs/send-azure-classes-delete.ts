...
@Injectable()
export class SendAzureClassesDeleteJob {
    constructor(private azureClassesService: AzureClassesService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_CLASSES_DELETE, { name: CONSTANTS.JOB_NAME_SEND_AZURE_CLASSES_DELETE })
    @RunOnDeployment({ names: [DeploymentGroup.SEND] })
    async handleCron() {
        if (this.previousJobHasFinished) {
            this.previousJobHasFinished = false;
            try {
                await this.azureClassesService.sendAzureClasses(WorkflowType.CLASS_DELETE);
            } catch (e) {
