...
@Injectable()
export class SendAzureEnrollmentsTeacherToClassDeleteJob {
    constructor(private azureEnrollmentsService: AzureEnrollmentsService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_ENROLLMENTS_TEACHER_TO_CLASS_DELETE, {
        name: CONSTANTS.JOB_NAME_SEND_AZURE_ENROLLMENTS_TEACHER_TO_CLASS_DELETE,
    })
