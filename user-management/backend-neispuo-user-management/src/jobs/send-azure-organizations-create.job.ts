...
@Injectable()
export class SendAzureOrganizationsCreateJob {
    constructor(private azureOrganizationsService: AzureOrganizationsService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_ORGANIZATIONS_CREATE, {
        name: CONSTANTS.JOB_NAME_SEND_AZURE_ORGANIZATIONS_CREATE,
    })
    ...
    async handleCron() {
        if (this.previousJobHasFinished) {
            ...
            try {
                await this.azureOrganizationsService.sendAzureOrganizations(WorkflowType.SCHOOL_CREATE);
            } catch (e) {
