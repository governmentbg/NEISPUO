...
@Injectable()
export class SendAzureOrganizationsDeleteJob {
    constructor(private azureOrganizationsService: AzureOrganizationsService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_ORGANIZATIONS_DELETE, {
        name: CONSTANTS.JOB_NAME_SEND_AZURE_ORGANIZATIONS_DELETE,
    })
    ...
    async handleCron() {
        if (this.previousJobHasFinished) {
            ...
            try {
                await this.azureOrganizationsService.sendAzureOrganizations(WorkflowType.SCHOOL_DELETE);
            } catch (e) {
