...
@Injectable()
export class SendAzureUsersDeleteJob {
    constructor(private azureUsersService: AzureUsersService) {}

    private previousJobHasFinished = true;

    @Cron(CONSTANTS.JOB_CRON_SEND_AZURE_USERS_DELETE, { name: CONSTANTS.JOB_NAME_SEND_AZURE_USERS_DELETE })
    @RunOnDeployment({ names: [DeploymentGroup.SEND] })
    async handleCron() {
        if (this.previousJobHasFinished) {
            this.previousJobHasFinished = false;
            try {
                await this.azureUsersService.sendAzureUsers(WorkflowType.USER_DELETE);
            } catch (e) {
