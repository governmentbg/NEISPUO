...
@Injectable()
export class ClassesErrorService {
    constructor(private telelinkService: TelelinkService) {}

    generateCreationErrorMessage(dto: AzureClassesResponseDTO, status: TelelinkCreateEventResponseEnum): string {
    ...
    generateCheckErrorMessage(dto: AzureClassesResponseDTO, status: TelelinkCheckStatusResponseEnum): string {
        const { telelinkResponseDto } = dto;
        switch (status) {
    ...
    generateSyncErrorMessage(dto: AzureClassesResponseDTO, status: WorkflowType) {
        const { telelinkResponseDto } = dto;
        //if in the future there is a need for a custom handling of a sync message. place it here
    }

    async handleFailedCreationErrorMessage(dtos: AzureClassesResponseDTO[], status: TelelinkCreateEventResponseEnum) {
    ...
    async handleFailedCheckErrorMessage(dtos: AzureClassesResponseDTO[], status: TelelinkCheckStatusResponseEnum) {
        for (const dto of dtos) {
    ...
    async handleFailedSyncErrorMessage(dtos: AzureClassesResponseDTO[], status: WorkflowType) {
        for (const dto of dtos) {
