...
@Injectable()
export class EnrollmentsErrorService {
    constructor(private telelinkService: TelelinkService) {}

    generateCreationErrorMessage(dto: AzureEnrollmentsResponseDTO, status: TelelinkCreateEventResponseEnum): string {
        ...
        switch (status) {
            ...
            case TelelinkCreateEventResponseEnum.ERROR:
                ...
                delete telelinkResponseDto?.response?.config?.headers;
    ...
    async handleFailedCreationErrorMessage(
        dtos: AzureEnrollmentsResponseDTO[],
    ...
    generateCheckErrorMessage(dto: AzureEnrollmentsResponseDTO, status: TelelinkCheckStatusResponseEnum): string {
        const { telelinkResponseDto } = dto;
        switch (status) {
    ...
    async handleFailedCheckErrorMessage(dtos: AzureEnrollmentsResponseDTO[], status: TelelinkCheckStatusResponseEnum) {
        for (const dto of dtos) {
