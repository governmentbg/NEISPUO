...
@Injectable()
export class OrganizationsErrorService {
    constructor(private telelinkService: TelelinkService) {}

    generateCreationErrorMessage(dto: AzureOrganizationsResponseDTO, status: TelelinkCreateEventResponseEnum): string {
    ...
    generateCheckErrorMessage(dto: AzureOrganizationsResponseDTO, status: TelelinkCheckStatusResponseEnum): string {
        const { telelinkResponseDto } = dto;
        switch (status) {
    ...
    generateSyncErrorMessage(dto: AzureOrganizationsResponseDTO, status: WorkflowType) {
        const { telelinkResponseDto } = dto;
        //if in the future there is a need for a custom handling of a sync message. place it here
    }

    async handleFailedCreationErrorMessage(
        dtos: AzureOrganizationsResponseDTO[],
        status: TelelinkCreateEventResponseEnum,
    ) {
    ...
    async handleFailedCheckErrorMessage(
        dtos: AzureOrganizationsResponseDTO[],
    ...
    async handleFailedSyncErrorMessage(dtos: AzureOrganizationsResponseDTO[], status: WorkflowType) {
        for (const dto of dtos) {
