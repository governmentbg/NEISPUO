...
@Injectable()
export class StudentUsersGuard implements CanActivate {
    ...
    private grantReadAccess(authObject: AuthObject, req: AuthedRequest) {
        const { isMonAdmin, isRuo, isMunicipality, isSchool } = authObject;
        const parsed = RequestQueryParser.create().parseQuery(req.query).getParsed();
        const scopeStudents = RequestQueryBuilder.create().search({
            $and: [
                {
                    sysRoleID: {
                        [CondOperator.EQUALS]: RoleEnum.STUDENT,
                    },
                },
                parsed.search,
            ],
