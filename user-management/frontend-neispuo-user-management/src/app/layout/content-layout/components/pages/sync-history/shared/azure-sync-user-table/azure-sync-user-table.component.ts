...
import { AzureSyncUserTableColumnsConfig } from './azure-sync-user-table-config';
import { AzureSyncTablesConfig } from '../config/azure-sync-tables.config';

@Component({
    selector: 'app-azure-sync-user-table',
    templateUrl: './azure-sync-user-table.component.html',
    styleUrls: ['./azure-sync-user-table.component.scss'],
    providers: [DynamicColumnService],
})
export class AzureSyncUserTableComponent implements OnInit, OnChanges {
    ...
    columnsConfig = AzureSyncUserTableColumnsConfig;

    columnServiceConfig: ColumnServiceConfig<AzureUsersResponseDTO> = {
        ...AzureSyncUserTableColumnsConfig,
        allColumns: AzureSyncUserTableColumnsConfig.allColumns as IColumn<AzureUsersResponseDTO>[],
        localStorageKey: 'azure-sync-user-table',
    };

    mergedData: AzureUsersResponseDTO[] = [];

    activeYearData: AzureUsersResponseDTO[] = [];

    archivedDataCache: { [year: string]: AzureUsersResponseDTO[] } = {};
    ...
    constructor(
        ...
        private columnService: DynamicColumnService<AzureUsersResponseDTO>,
    ) {}
    ...
    private loadActiveYear(): void {
        ...
        this.syncHistoryService
            .getActiveAzureUserData(Number(this.personID))
            .pipe(
                finalize(() => {
                    this.loading = false;
                }),
    ...
    private loadArchivedYears(years: string[]): void {
        ...
        const requests = years
            ...
            .map((year: string) =>
                this.syncHistoryService
                    .getArchivedAzureUserData({ identifier: this.personID, schoolYear: year })
                    .toPromise()
                    .then((data) => {
    ...
    private mergeData(): void {
        ...
        const archived = archivedYears.reduce((acc: AzureUsersResponseDTO[], year: string) => {
    ...
    restartWorkflow(rowID: number): void {
        this.azureUsersService.restartWorkflow(rowID).subscribe({
            next: () => {
                this.loadActiveYear();
    ...
    trackByUserID(index: number, item: AzureUsersResponseDTO) {
        return item.userID;
    }
