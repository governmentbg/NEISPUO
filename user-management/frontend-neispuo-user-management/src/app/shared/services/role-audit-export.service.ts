...
import { RoleAudit } from 'src/app/layout/content-layout/components/pages/update-roles-page/role-audit/role-audit';
...
export class RoleAuditExportService {
    tableVisibleColumns: IColumn<RoleAudit>[] = [];

    constructor(
        private excelExportService: ExcelExportService,
        private columnService: DynamicColumnService<RoleAudit>,
    ...
    private createExportRows(
        ...
        for (const ra of roleAudit) {
            ...
            for (const exportColumn of columnsToExport) {
                ...
                const fieldData = getDataMethod(this.getModifiedEntry(ra), exportColumn.dottedPropName);
    ...
    createExcel(queryParams: any, getDataMethod: (value: any, dotNotatedProperty: string) => string) {
        return this.apiService.get('/v1/role-assignment', queryParams).pipe(
    ...
    private getModifiedEntry(roleAudit: RoleAudit) {
        return {
            ...roleAudit,
