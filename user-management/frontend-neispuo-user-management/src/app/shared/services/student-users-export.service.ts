...
export class StudentUsersExportService {
    tableVisibleColumns: IColumn<StudentResponseDTO>[] = [];

    constructor(
        private excelExportService: ExcelExportService,
        private columnService: DynamicColumnService<StudentResponseDTO>,
    ...
    private createExportRows(
        ...
        for (const student of studentResponseDTO) {
    ...
    createExcel(queryParams: any, getDataMethod: (value: any, dotNotatedProperty: string) => string) {
        return this.apiService.get('/v1/student-users', queryParams).pipe(
